...
struct BST {
		...
		// Funtions to find all nodes and print the deepest node
		int height(BTNode *cur);
		bool deepestNodes();
		void deepestNodeSecond(BTNode *, int , int );
		// Funtions to print student record 
		bool display(int , int );
		void displaySecond(BTNode*, int , int , ofstream &);
		// Functions to clone subtree
		bool CloneSubTree(BST, type);
		bool CloneSubTree2(BTNode* root, BTNode* cur, int key);
		// Functions to print nodes according to their level
		bool printLevelNodes();
		// Functions to print paths
		bool printPathsRecur(BTNode* ,int [],int ); 
		bool printDataArray(int [], int);
		bool printPath();
